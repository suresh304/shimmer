...
const Searchbar = () => {
    ...
    const fetchData = async () => {
        ...
        if (cache[searchText]) {
            ...
            const data = await fetch(`https://www.google.co.in/complete/search?q=${searchText}&client=firefox`)
    ...
    return (
        <center>
            ...
            <div className='h-14 w-[600px] bg-white p-5 flex items-center justify-between rounded-md border '>
                <input className='border border-none bg-white outline-none w-full'
                    value={searchText}
            ...
            <ul className='w-[600px] border shadow-lg'>
                {suggestions.length > 0 && showSuggestion && suggestions.map((sug) => <li className='h-12 hover:bg-slate-200 flex items-center pl-8' >{sug}</li>)}
